...
`Zend\DataValidator\Barcode` allows you to check if a given value can be represented
as a barcode.
...
```php
$validator = new Zend\DataValidator\Barcode(new Zend\DataValidator\Barcode\EAN13());
...
You may write custom barcode validators for usage with `Zend\DataValidator\Barcode`;
...
Your custom barcode validator must extend `Zend\DataValidator\Barcode\AbstractAdapter`
or implement `Zend\DataValidator\Barcode\AdapterInterface`.
